...
from .visualization import (
    ...
    plot_predicted_vs_actual_panels_grouped_unseen,
...
def run() -> None:
    ...
    plot_predicted_vs_actual_panels_grouped_unseen(prediction_df=prediction_df, results_df=grouped_results)
    plot_predicted_vs_actual_panels_grouped_unseen_v2(prediction_df=prediction_df, results_df=grouped_results)
    plot_model_comparison_r2_grouped_clean(grouped_results)
    ...
    audit_lines = [
        ...
        '- Audited `predicted_vs_actual_panels_final` and confirmed it was not updated after the grouped unseen-session correction.',
        '- Generated a new grouped-evaluation predicted-vs-actual figure: `predicted_vs_actual_panels_grouped_unseen_sessions.png`.',
        '- Audited negative R² values in grouped evaluation and confirmed they are real outputs, not plotting bugs.',
        '- Generated a cleaner grouped R² chart with improved layout and explicit negative-R² context: `model_comparison_r2_grouped_unseen_sessions_v2.png`.',
        '- Audited model-selection consistency between grouped model-comparison results and grouped predicted-vs-actual panels.',
        '- Generated a new consistently selected panel figure: `predicted_vs_actual_panels_grouped_unseen_sessions_v2.png`.',
        '',
        ...
        '- `outputs/figures/predicted_vs_actual_panels_final.png` was generated by the older main pipeline and is not a grouped unseen-session evaluation artifact.',
        ...
        '',
        '## Grouped best-model selection rule',
        '',
        'For the grouped unseen-session predicted-vs-actual panels, the displayed model for each target is selected using one consistent rule from `grouped_unseen_session_results.csv`:',
        '',
        '- primary criterion: highest R²',
        '- tie-break criterion: lowest RMSE',
        '',
        'Selected grouped best models under this rule:',
        '',
        '- `urea` → `svr_rbf` with `all_absorbance_plus_machine_plus_personal`',
        '- `creatinine` → `svr_rbf` with `all_absorbance_only`',
        '- `acid_inferred_uric_acid` → `linear_regression` with `all_absorbance_only`',
        '- `phosphorus` → `linear_regression` with `all_absorbance_only`',
        '- `potassium` → `linear_regression` with `all_absorbance_only`',
        '- `beta2_microglobulin` → `random_forest` with `all_absorbance_plus_machine_plus_personal`',
        '',
        'Linear regression therefore remains selected for some targets in the grouped unseen-session panels because it truly has the best grouped R² under the corrected evaluation protocol for those targets, not because the old pipeline leaked into the figure.',
        '',
        ...
        '',
        'Negative R² values in grouped evaluation are valid when a model performs worse than a simple mean predictor on unseen-session data. These values are especially visible for some linear-regression configurations under stricter grouped evaluation and are not caused by a metric or plotting bug.',
    ]
    ...
    correction_lines = [
        ...
        '- Figure: `outputs/figures/predicted_vs_actual_panels_grouped_unseen_sessions.png`',
        '- Figure: `outputs/figures/model_comparison_r2_grouped_unseen_sessions.png`',
        '- Figure: `outputs/figures/model_comparison_r2_grouped_unseen_sessions_v2.png`',
        ...
        '- `outputs/figures/predicted_vs_actual_panels_final.png`',
        ...
        '10. Audited `predicted_vs_actual_panels_final` and generated a grouped unseen-session version to keep evaluation logic consistent.',
        '11. Audited grouped negative R² values and confirmed they are valid outputs, then generated a cleaner grouped R² chart with improved layout and explicit explanation.',
        '12. Audited grouped panel/model consistency and generated a versioned predicted-vs-actual grouped panel figure with explicit best-model selection logic.',
        '',
        ...
        '- `predicted_vs_actual_panels_final.png` belonged to the older main pipeline and was not updated after the grouped unseen-session correction.',
        '- The first grouped unseen-session R² chart could be visually incomplete and could mislead readers if negative R² values were not explained.',
        '- The grouped predicted-vs-actual panel needed an explicit statement of the model-selection rule to avoid apparent inconsistency with the grouped model-comparison chart.',
        '',
        ...
        '- Were the model-performance figures updated? **Yes — corrected grouped-evaluation figures were generated with new filenames, including a cleaner v2 grouped R² chart and a grouped unseen-session predicted-vs-actual panel figure. Older non-corrected artifacts remain only as legacy traceability outputs.**',
